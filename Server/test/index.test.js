   ...
   describe( "Test de RUTAS", () =>{
    describe("GET /rickandmorty/character/:id", () => {
        ...
        it("Si hay un error responde con status: 500", async () =>{
            const response = await request.get("/rickandmorty/character/3425k");
    ...
    describe("GET /rickandmorty/login", () => {
        it("Responde con un objeto con la propiedad access en true si la información del usuario es válida", async () => {
            const response = await request.get("/rickandmorty/login?email=clau@gmail.com&password=123clau");
            const access = { access: true };
            expect(response.body).toEqual(access);

        });
        it("Responde con un objeto con la propiedad access en false si la información del usuario no es válida", async () => {
            const response = await request.get("/rickandmorty/login?email=clau@mail.com&password=123crau");
    ...
    describe("POST /rickandmorty/fav", () => {
        ...
        it("Debe agregar personajes a favoritos sin eliminar los que ya existían", async () => {
            character.id = 1923;
            character.name = "FT 42a";
            const response = await request.post('/rickandmorty/fav').send(character);
